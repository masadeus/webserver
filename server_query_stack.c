    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            if (query_bg != NULL) 
            {
                ...
                {
                    char query_t[query_ln]; 
                    memset(query_t, 0, query_ln);
                    strcpy(query_t, query_bg + 1); // si no funciona provar strncpy
                    query_t[query_ln - 1] = '\0'; // append NULL in the end
                    query = query_t;
            ...
            int path_ln = strlen(root) + ln_abs_path - 1;
            char path[path_ln];  // aquí està l'error
            memset(path, 0, path_ln);
            strcpy (path, root);
